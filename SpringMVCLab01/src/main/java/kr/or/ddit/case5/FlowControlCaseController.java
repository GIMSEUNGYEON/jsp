...
public class FlowControlCaseController {
	...
	@GetMapping("dest1")
	public String handler1_dest(@RequestAttribute(value = "now", required = false) LocalDateTime now) {
		log.info("전달된 model data : {}", now);
	...
	public String handler2_dest(Model model) {
		log.info("전달된 model data : {}", model.getAttribute("now"));
